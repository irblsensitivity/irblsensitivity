...
class GitLog:
	...
	def make(self):
		...
		command = [u'git', u'log', u'--name-status', u'--pretty=format:---------------------%nhash:%h%nauthor:%an%ncommit_date:%ci%nmessage:%s%n']
	...
	def file_loader(self, _f):
		...
		for line in _f:
			...
			if line.startswith(u'message:'):
				...
				continue
			if line.startswith(u'M\t') or line.startswith(u'D\t'):
				fname = line[2:-1].strip()
				ftype = line[:1]
				if self.filetype_fileter(fname) is False: continue
	...
	def load(self):
		...
		:return:
		'''
		if os.path.exists(self.GitLogPath) is False:
			self.make()
